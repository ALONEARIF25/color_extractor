...
function handleImageUpload(event) {
  ...
  img.onload = () => {
    ...
    const middleColor = sortedColors[Math.floor(sortedColors.length / 2)];

    // Set styles based on the top 4 colors
    document.body.style.backgroundColor = sortedColors[0];
    document
      .querySelectorAll("h1, h2")
      .forEach((h) => (h.style.color = sortedColors[3]));
    document
      .querySelectorAll("h3, h4")
      .forEach((h) => (h.style.color = getComplementaryColor(sortedColors[0])));
    document.querySelectorAll("label").forEach((l) => {
      l.style.backgroundColor = sortedColors[4];
      l.style.color = sortedColors[0];
    });
    ...
    // Display sorted colors and complementary colors
